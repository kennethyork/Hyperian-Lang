...
sed 's/toolchain: 0.45.0/toolchain: 0.1.0/' "$pack/hyperian.runtime" > "$stale/hyperian.runtime"
...
printf '%s\n' "$output"
printf '%s\n' "$output" | grep -F "uses Hyperian 0.1.0 but this compiler is Hyperian 0.45.0" >/dev/null
test ! -e "$application"
