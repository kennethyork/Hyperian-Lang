...
On Linux and macOS, the compiler and VM are now at `build/hyperian`. With the standard Visual Studio generator on Windows, they are at `build/Release/hyperian.exe`. Hyperian uses native Windows files, processes, timers, and Winsock networking; it does not need Python or a Unix compatibility runtime.
...
`hyperian platform` prints a precise name such as `linux-x64`, `linux-arm64`, `macos-x64`, `macos-arm64`, or `windows-x64`. `to` creates an inspectable development folder. `as` creates one downloadable `.hyr` archive. Move either form to the development computer. The compiler consumes an archive directly without a separate extraction command:
