...
A bundle carries an `HYBN1` manifest recording its toolchain, native platform, executable, and asset folders. Its executable automatically uses the bundle folder as its working directory, so file access, game media, and public web assets work even when the program is launched elsewhere. Without a runtime pack, a bundle targets the operating system and CPU on which the compiler was built. Native system libraries are still external dependencies.
...
hyperian pack runtime as hyperian-linux-arm64.hyr
```

`hyperian platform` prints a precise name such as `linux-x64`, `linux-arm64`, `macos-x64`, or `macos-arm64`. `to` creates an inspectable development folder. `as` creates one downloadable `.hyr` archive. Move either form to the development computer. The compiler consumes an archive directly without a separate extraction command:

```sh
hyperian build app.hyp for linux-arm64 using hyperian-linux-arm64.hyr as MyApp
```

Applications that need `assets/` or `public/` can become a complete release folder for the destination in one command:

```sh
hyperian bundle app.hyp for linux-arm64 using hyperian-linux-arm64.hyr to MyAppBundle
```

This produces `MyAppBundle/run` (or `run.exe` for a Windows runtime pack), copies both asset trees recursively, and records the destination in `hyperian.bundle`. The packaged executable enters its own bundle folder before running, so relative asset paths work no matter where it was launched. Bundle creation rejects symbolic links and unsupported filesystem entries instead of silently packaging files outside the project, and a failed operation removes its incomplete output.
...
Tagged releases publish tested `.hyr` archives and separate SHA-256 files for Linux x64, Linux arm64, macOS x64, and macOS arm64 on the [GitHub releases page](https://github.com/kennethyork/Hyperian-Lang/releases). Each native runner builds Hyperian, runs its test suite, confirms its real platform name, creates its own archive, and uploads it. The release job verifies all four external checksums before publishing them.
