...
The project embeds `application.hyc` and Hyperian's C runtime. An Objective-C bridge exposes the runtime to Swift through the project's bridging header, and SwiftUI renders the current MVC view using native controls. English web requests use URLSession's native HTTP/HTTPS support. The project links SQLite for `store data in sqlite file`, while HDB remains available; both use the application's private `hyperian-data.db` path.
